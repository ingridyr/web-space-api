...
router.patch("/:id/photo", multer(multerConfig).single("file"), (req, res) =>
  UserControllers.updatePhotoUrl(req, res)
);
router.get("/:id", (req, res) => UserControllers.readOneUser(req, res));
router.patch("/:current/follow/:target", (req, res) =>
  UserControllers.follow(req, res)
);
